...
function create () {
    ...
    y = 50;
    for (let j = 0; j < 4; j++) {
        rect = self.add.rectangle(0, 0, 200, 100);
        let container = self.add.container(screen.width - 100, y, 200, 100, [rect2]);        
        ...
        var hitArea2 = new Phaser.Geom.Rectangle(0, 0, 150, 70);
        c2.setInteractive(hitArea2, Phaser.Geom.Rectangle.Contains);
        

        self.physics.add.collider(container, c2);
    ...
    rect1 = self.add.rectangle(0, 0, 150, 70, 0xd7eef2);

    rect2 = self.add.rectangle(0, 0, 150, 70, 0x000000);

    rect.setAlpha(1);

    let container = self.add.container(50, 100, [rect]);
    let c2 = self.add.container(50, 250, [rect2]);

    container.setSize(150,70);
    c2.setSize(150, 70);

    self.physics.world.enable(container);
    self.physics.world.enable(c2);

    var hitArea = new Phaser.Geom.Rectangle(0, 0, 150, 70);
    container.setInteractive(hitArea, Phaser.Geom.Rectangle.Contains);
