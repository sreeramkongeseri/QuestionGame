...
function createWords (phrase) {
  ...
  let phrase_text = self.add.text(0, 0, phrase['Question'], style);
  phrase_text.setOrigin(0.5);
  ...
  let phrase_cont = self.add.container(x_coord, y_coord, [phrase_rect, phrase_text]);
  phrase_cont.setSize(text_cont_width, text_cont_height);
  
  phrase_cont.phrase = phrase;
  ...
  self.input.on('dragend', function (pointer, gameObject) {
    ...
    let value = gameObject.phrase.phrase.value;
  
    if (val != null) {
      if (!(gameObject.x == steps[val - 1].x) && (gameObject.y == steps[val - 1].y)) {
        colliders[gameObject.phrase["No"] - 1][steps[val-1].id - 1].active = true;
      }
    }
...
function done (phrase_cont, step) {
  ...
  if (!dragStart) {
    ...
    phrase_cont.phrase.value = step.id;
    colliders[phrase_cont.phrase["No"] - 1][step.id - 1].active = false;
  }
}
