...
/* Game Settings */
const colors = [0x1a75ff, 0x4d94ff, 0x80b3ff, 0xb3d1ff, 0xe6f0ff];
...
function create () {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    let rect = self.add.rectangle(0, 0, step_widths[i], step_height, colors[i]);
...
$(document).ready(function() {
  ...
  $('#submit').on('click',  () => {
    ...
    steps.forEach(s => {
      s.hasPhrase = false;
      s.list[0].setFillStyle(0x000000);
    });

    cur_screen++;
    createQuestions(quesData.slice(5*(cur_screen - 1), 5*cur_screen));
  });
});
