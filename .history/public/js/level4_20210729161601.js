...
function createWords (phrase_obj) {

  phrase_num = phrase_obj['No'];
  phrase_text = phrase_obj['Question'];
  phrase_dok = phrase_obj['DoK'];
  phrase_obj.value = null;
  ...
  phrase_cont.num = phrase_num;
  phrase_cont.dok = phrase_dok;

  console.log('(' + x_coord + ',' + y_coord + ')');
  console.log((config.width - step_widths[3])/2 - (text_cont_width/2) - 5);
  ...
  self.input.on('dragend', function (pointer, gameObject) {
    ...
    let val = phrase[parseInt(gameObject.num) - 1].value;
    if (val != null) {
      if (!(gameObject.x == steps[val - 1].x) && (gameObject.y == steps[val - 1].y)) {
        colliders[parseInt(phrase_cont.num) - 1][steps[val-1].id - 1].active = true;
      }
    }
  ...
  steps.forEach(step => {
    ...
    let collilder = self.physics.add.overlap(phrase_cont, step, done, null, this);
    c.push(collider);
  });
...
function done (phrase_cont, step) {
  ...
  if (!dragStart) {
    ...
    phrases[parseInt(phrase_cont.num) - 1].value = step.id;
    colliders[parseInt(phrase_cont.num) - 1][step.id - 1].active = false;
  }
}
