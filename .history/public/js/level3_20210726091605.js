...
function create () {
    ...
    y = 50;
    for (let j = 0; j < 4; j++) {
        var rect = self.add.rectangle(0, 0, 200, screen.height/4 , colors[i]);
        let container = self.add.container(screen.width - 100, y,[rect]);        
        container.setSize(200,100);
        self.physics.world.enable(container);
        hitArea = new Phaser.Geom.Rectangle(0, 0, 200,100);
