...
function create () {
    ...
    for (let j = 0; j < 1; j++) {
        ...
        var bucketLabel = self.add.text(-30, 0, ''+(j+1), style);
        var rect = self.add.rectangle(0, 0, 200, config.height/4 , colors[j]);
        let container = self.add.container(config.width - 50, 100,[rect, bucketLabel]);        
        container.setSize(200, config.height/4);
