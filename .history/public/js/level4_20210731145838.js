...
function done (phrase_cont, step) {
  ...
  if (!dragStart && phrase_cont.value == null && !step.phraseNum) {
...
$(document).ready(function() {
  ...
  $('#submit').on('click',  () => {
    ...
    let order = steps.map(s => findDok(s.hasPhrase)
      
      parseInt(phrase_conts[s.hasPhrase - 1].dok));
