...
const step_factor = 0.8

function preload () {
  
  step_widths[0] = config.width * step_factor;
  for (let i = 1; i < 6; i++) { 
    step_widths[i] = step_widths[i-1] * step_factor;
  }
}
...
function createWords (phrase_text, phrase_dok) {
  ...
  let phrase_cont = self.add.container(70, 70, [phrase_rect, phrase]);
