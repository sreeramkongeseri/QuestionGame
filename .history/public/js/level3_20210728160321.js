...
function create () {
    ...
    for (let j = 0; j < 4; j++) {
        ...
        let container = self.add.container(config.width - targetWidth, y,[target, channelLabel]);        
...
function createWords(phrase_text, phrase_dok) {
    ...
    style = {fontFamily: 'Helvetica', fontSize: "15px", color: "#000000", wordWrap: {width: 300, useAdvancedWrap: true}}
    phrase = self.add.text(0, 0, phrase_text, style)
    
    rect = self.add.rectangle(0, 0, rectWidth, rectHeight, 0xd7eef2);
    rect.setAlpha(0);

    let container = self.add.container(-10, Phaser.Math.Between(70, config.height - 25), [ rect, word]);
    container.name = phrase_text;
