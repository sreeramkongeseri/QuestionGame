...
const text_cont_width = 300;
const text_cont_height = 150;
...
function createWords (phrase_text, phrase_dok) {
  ...
  let x_lim = (config.width - step_widths[3])/2 - 20;
  let x_coord = Phaser.Math.Between(0, x_lim);
  if (Math.random() > 0.5) { 
    x_coord = config.width - x_coord;
  }
  let y_lim = steps[3].y - 20;

  let phrase_cont = self.add.container(x_coord, Phaser.Math.Between(0, y_lim), [phrase_rect, phrase]);
