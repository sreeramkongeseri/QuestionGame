...
function createWords (phrase_text, phrase_dok) {
  ...
  phrase.setWordWrapWidth(wrap_width);
  phrase.setOrigin(0.5).refreshBody();
  ...
  let x_coord = Phaser.Math.Between(text_cont_width/2, (config.width - step_widths[3])/2 - (text_cont_width/2));
  let y_coord = Phaser.Math.Between( text_cont_height/2, steps[3].y);
  ...
  let phrase_cont = self.add.container(0, 0, [phrase_rect, phrase]);
