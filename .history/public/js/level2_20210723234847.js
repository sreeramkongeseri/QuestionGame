...
function create ()
{
    ...
    // wordData = fetch('api/level2').then(response => response.json()).then(verbData =>
    //     createWords(verbData, self)
    // );

    wordNames = [{"think": '1'}, {'classify': '2'}, {'enjoy': '3'}];
    ...
    id = setInterval( () => { 
        ...
        createWords(wordName, wordBucket, self);

    }, 5000);

    // id = setInterval((verbData, self) => {
