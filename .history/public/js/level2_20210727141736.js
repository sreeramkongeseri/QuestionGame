...
function create () {
    ...
    bins = this.physics.add.staticGroup({
        key: 'bin',
        repeat: 5,
        setXY: { x: 100, y: config.height + 30, stepX: config.width/6 }
    });

    names = ["knowledge", "comprehension", "application", "analysis","synthesis", "evaluation"];
    locs = [50, 215, 425, 625, 810, 992];

    let i = 1;
    bins.children.iterate(function (child) {

        channelWidth = config.width/6;
        scale = (channelWidth - 20)/child.width;
        child.setScale(scale, 0.35).refreshBody();
        child.setX(10 + (child.width/2))
        console.log(child);
        child.name = "" + i;
        label = self.add.text(locs[i-1], 700, names[i-1], {font: '22px Helvetica', color: '#ffffff'});
        i += 1;
    });
