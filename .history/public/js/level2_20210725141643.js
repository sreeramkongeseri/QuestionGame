...
function create () {
    ...
    bins = this.physics.add.staticGroup({
        key: 'bin',
        repeat: 1,
        setXY: { x:0, y: 50, stepX: screen.width/6 }
    });
    ...
    bins.children.iterate(function (child) {
        ...
        child.name = "" + i;
        label = self.add.text(0, -10, child.name, {font: '25px Helvetica'});
        container = self.add.container(100, , [ child, label ]);
