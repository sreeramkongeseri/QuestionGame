...
function createWords (phrase) {
  ...
  let j = 0;
  while (text_cont_width > (config.width - step_widths[j])/2) {
    j++;
  }
  console.log(j);
  let y_coord = Phaser.Math.Between(text_cont_height/2, steps[j].y);
...
function done (phrase_cont, step) {
  ...
  if (!dragStart && phrase_cont.value == null) {
    phrase_cont.setX(step.x);
    phrase_cont.setY(step.y);
    phrase_cont.value = step.id;
    colliders[step.id].active = false;

    let style = {fontSize: "20px", fontStyle: "bold", wordWrapWidth: {width: step.width - 10, useAdvancedWrap: true}};
    let text = self.add.text(0, 0, phrase_cont.text, style);

    phrase_cont.list[1] = text;

 //   phrase_cont.setSize()

   // phrase_cont.list[1].setFontSize(20).setFontStyle("bold").setWordWrapWidth(step.width - 10, true);
 //   phrase_cont.list[0].setSize(phrase_cont.list[1].width + 30, text_cont_height).setX(0).setY(0);
    phrase_cont.setSize(phrase_cont.list[1].width + 30, text_cont_height);

    phrase_cont.list[0].setX(0).setY(0);
    phrase_cont.list[1].setX(0).setY(0);
    var hitArea = new Phaser.Geom.Rectangle(0, 0, phrase_cont.list[1].width + 30, text_cont_height);
    phrase_cont.setInteractive(hitArea, Phaser.Geom.Rectangle.Contains);
